  ...
  return <section id="about" className="about-profile section" aria-labelledby="profile-heading">
    ...
    <div className="profile-layout">
      ...
      <div className="profile-information">
        ...
        <Reveal><section className="profile-block profile-tools" aria-labelledby="tools-heading"><h4 id="tools-heading">TOOLS</h4><ul className="profile-software-grid">{tools.map(item => <li key={item.icon}><img src={`/media/tools/${item.icon}.svg`} alt="" width={72} height={72} loading="lazy"/><span>{item.name}</span></li>)}</ul></section></Reveal>
      </div>
    </div>
  </section>;
}
