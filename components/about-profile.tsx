...
const capabilities = ['AI 视觉设计', '数字人', 'AI 视频', '分镜脚本', '电商设计', '创意策划'];
const tools = [{name:'Photoshop',icon:'photoshop'},{name:'Premiere Pro',icon:'premiere'},{name:'After Effects',icon:'after-effects'},{name:'Adobe Illustrator',icon:'illustrator'},{name:'剪映',icon:'jianying'},{name:'AutoCAD',icon:'autocad'}];
  ...
  return <section id="about" className="about-profile section" aria-labelledby="profile-heading">
    ...
    <div className="profile-layout">
      ...
      <div className="profile-information">
        ...
        <Reveal><section className="profile-block profile-tools" aria-labelledby="tools-heading"><h4 id="tools-heading">TOOLS</h4><ul className="profile-software-grid">{tools.map(item => <li key={item.icon}><img src={`/media/tools/${item.icon}.${["jianying","autocad"].includes(item.icon)?"png":"svg"}`} alt="" width={72} height={72} loading="lazy"/><span>{item.name}</span></li>)}</ul></section></Reveal>
      </div>
    </div>
  </section>;
}
